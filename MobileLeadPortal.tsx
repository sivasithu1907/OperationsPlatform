...
// --- MAIN COMPONENT ---
const MobileLeadPortal: React.FC<MobileLeadPortalProps> = ({ 
  ...
  // Modals State
  const [modalType, setModalType] = useState<'dispatch' | 'cancel' | 'carry' | 'job_carry' | 'job_complete' | null>(null);
  ...
  const TeamView = () => {
      return (
          <div className="p-4 space-y-3 pb-24">
              ...
              {technicians.filter(t => t.isActive !== false && t.systemRole === Role.TEAM_LEAD && t.status !== 'LEAVE').map(tech => {
          ...
          <div className="h-full overflow-y-auto custom-scrollbar pb-24">
              ...
              {activeTab === 'my_jobs' && (
                  <div className="p-4 space-y-4">
                      <h3 className="text-xs font-bold text-slate-500 uppercase mb-2 px-1">My Assigned Jobs ({myJobs.length})</h3>
                      {myJobs.length === 0 && <p className="text-center text-slate-400 text-sm py-8">No jobs assigned to you</p>}
                      {myJobs.map(t => <JobCard key={t.id} ticket={t} />)}
                  </div>
              )}
            ...
            {modalType === 'dispatch' && modalTicket && (
                ...
                >
                    ...
                    >
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Field Engineer</label>
                            <select
